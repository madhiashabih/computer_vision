...
def process_matches(matches: np.ndarray, src_img: np.ndarray, max_distance: float = 400) -> np.ndarray:
...
def compute_fundamental_matrix(src_img, matches: np.ndarray, threshold: float = 1, iters: int = 2000) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    plot_matches(src_img, inliers, max_distance=400)
    return inliers_1, inliers_2, F

def compute_essential_matrix(F: np.ndarray, K: np.ndarray, K2: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
